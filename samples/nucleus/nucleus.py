...
VAL_IMAGE_IDS = [
    "G04_120919_100",
    "G04_120919_100_EnhanceSharpness",
...
class NucleusDataset(utils.Dataset):
    ...
    def load_nucleus(self, dataset_dir, subset):
        ...
        for image_id in image_ids:
            images_dir = os.path.join(dataset_dir, image_id, "images")
            mnt_dir = os.path.join(dataset_dir, image_id, "mnt")

            # Check which directory exists
            if os.path.exists(images_dir):
                image_files_dir = images_dir
            elif os.path.exists(mnt_dir):
                image_files_dir = mnt_dir
            else:
                print(f"Neither 'images' nor 'mnt' directory found for {image_id}")
                continue

            print(f"Checking directory: {image_files_dir}")
            try:
                img_files = next(os.walk(image_files_dir))[2]
                print(f"Found image files: {img_files}")
            except StopIteration:
                print(f"Directory not found or empty: {image_files_dir}")
                continue
            ...
            for img_file in img_files:
                if img_file.endswith(".png"):
                    self.add_image(
                        ...
                        path=os.path.join(image_files_dir, img_file))
    ...
    def load_mask(self, image_id):
        ...
        for f in next(os.walk(mask_dir))[2]:
            if f.endswith(".png"):
                m = skimage.io.imread(os.path.join(mask_dir, f)).astype(np.bool_)
                print(f"Loaded mask shape: {m.shape} for file {f}")
                # Convert multi-channel mask to single channel if needed
                if m.ndim == 3:
                    m = m[:, :, 0]  # Assuming the first channel is the mask
                mask.append(m)

        if not mask:
            print(f"No masks found for image {image_id}")
            return np.empty((0, 0, 0)), np.empty((0,), dtype=np.int32)

        mask = np.stack(mask, axis=-1)
        print(f"Stacked mask shape: {mask.shape}")

        # Ensure mask is three-dimensional
        assert mask.ndim == 3, f"Mask has invalid dimensions: {mask.ndim}"
